...
function createSettingsSheet(ss) {
  ...
  sheet.getRange('A1:C1').setValues([['Parameter', 'Gasoline', 'Diesel']]).setFontWeight('bold');
...
function createFuelPurchasesSheet(ss) {
  ...
  const headers = [
    ...
    'Exchange Rate (USD to KHR)', 'Total Buy (USD)', 'Total Buy (KHR)',
    'Liters', 'Tanks', 'Buy per Liter (USD)', 'Buy per Liter (KHR)',
    'Buy per Tank (USD)', 'Buy per Tank (KHR)'
  ];
  sheet.getRange('A1:N1').setValues([headers]).setFontWeight('bold');
  sheet.setColumnWidth(6, 200);

  // Define the effective exchange rate: use custom if available, otherwise default
  const effectiveRate = 'IF(F2<>"", F2, Settings!$B$6)';

  // Set formulas for calculated columns
  sheet.getRange('G2').setFormula(`=IF(D2="USD", C2*E2, (C2*E2)/${effectiveRate})`);
  sheet.getRange('H2').setFormula(`=IF(D2="KHR", C2*E2, (C2*E2)*${effectiveRate})`);
  sheet.getRange('I2').setFormula('=E2*IF(B2="Gasoline",Settings!B2,Settings!C2)');
  sheet.getRange('J2').setFormula('=I2/IF(B2="Gasoline",Settings!B3,Settings!C3)');
  sheet.getRange('K2').setFormula('=G2/I2');
  sheet.getRange('L2').setFormula('=H2/I2');
  sheet.getRange('M2').setFormula('=K2*IF(B2="Gasoline",Settings!B3,Settings!C3)');
...
function createFuelSalesSheet(ss) {
  ...
  // Define formulas for calculated columns
  const avgBuyPrice = "AVERAGEIF('Fuel Purchases'!B:B, B2, 'Fuel Purchases'!N:N)";
  const exchangeRate = "Settings!$B$6";

  sheet.getRange('E2').setFormula(`=${avgBuyPrice}+D2`);
  sheet.getRange('F2').setFormula("=E2/IF(B2=\"Gasoline\",Settings!B3,Settings!C3)");
  sheet.getRange('G2').setFormula("=E2*C2");
  sheet.getRange('H2').setFormula(`=G2/${exchangeRate}`);
  sheet.getRange('I2').setFormula("=D2*C2");
  sheet.getRange('J2').setFormula(`=I2/${exchangeRate}`);
}
...
function createDashboardSheet(ss) {
  ...
  sheet.getRange('A1:B1').setValues([['Metric', 'Value']]).setFontWeight('bold');
  ...
  sheet.getRange('B2').setFormula("=SUMIF('Fuel Purchases'!B:B,\"Gasoline\",'Fuel Purchases'!G:G)");
  sheet.getRange('B3').setFormula("=SUMIF('Fuel Purchases'!B:B,\"Diesel\",'Fuel Purchases'!G:G)");
  sheet.getRange('B4').setFormula("=SUMIF('Fuel Purchases'!B:B,\"Gasoline\",'Fuel Purchases'!H:H)");
  sheet.getRange('B5').setFormula("=SUMIF('Fuel Purchases'!B:B,\"Diesel\",'Fuel Purchases'!H:H)");
